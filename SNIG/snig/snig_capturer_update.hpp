...
template <typename T>
class SNIGCapturerUpdate : public Base<T> {
...
// ----------------------------------------------------------------------------
// Definition of SNIGCapturerUpdate
// ----------------------------------------------------------------------------

template <typename T>
SNIGCapturerUpdate<T>::SNIGCapturerUpdate(
  const dim3& threads,
  const std::fs::path& weight_path,
  const T bias,
  ...
  Base<T>(threads, weight_path, bias, num_neurons_per_layer, num_layers)
{
  ...
  Base<T>::log("Constructing SNIGCapturerUpdate engine......", "\n");
}
...
template <typename T>
Eigen::Matrix<int, Eigen::Dynamic, 1> SNIGCapturerUpdate<T>::infer(
  const std::fs::path& input_path,
  const size_t num_inputs,
  const size_t batch_size,
  const size_t num_weight_buffers,
...
template <typename T>
void SNIGCapturerUpdate<T>::_set_parameters(
  const size_t num_inputs,
  const size_t batch_size,
  const size_t num_weight_buffers,
  const size_t num_gpus
...
template <typename T>
void SNIGCapturerUpdate<T>::_preprocess(const std::fs::path& input_path) {
...
void SNIGCapturerUpdate<T>::_infer() {
  ...
  std::vector<std::unique_ptr<tf::cudaFlowCapturer>> cfs;
  for(size_t i = 0; i < Base<T>::_num_gpus; ++i) {
    cfs.emplace_back(std::make_unique<tf::cudaFlowCapturer>());
  }
...
template <typename T>
void SNIGCapturerUpdate<T>::_weight_alloc() {
...
template <typename T>
void SNIGCapturerUpdate<T>::_input_alloc() {
...
template <typename T>
void SNIGCapturerUpdate<T>::_result_alloc() {
