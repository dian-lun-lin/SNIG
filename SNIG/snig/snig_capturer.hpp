...
template <typename T>
class SNIGCapturer : public Base<T> {
...
// ----------------------------------------------------------------------------
// Definition of SNIGCapturer
// ----------------------------------------------------------------------------

template <typename T>
SNIGCapturer<T>::SNIGCapturer(
  const dim3& threads,
  const std::fs::path& weight_path,
  const T bias,
  ...
  Base<T>(threads, weight_path, bias, num_neurons_per_layer, num_layers)
{
  ...
  Base<T>::log("Constructing SNIGCapturer engine......", "\n");
}
...
template <typename T>
Eigen::Matrix<int, Eigen::Dynamic, 1> SNIGCapturer<T>::infer(
  const std::fs::path& input_path,
  const size_t num_inputs,
  const size_t batch_size,
  const size_t num_weight_buffers,
...
template <typename T>
void SNIGCapturer<T>::_set_parameters(
  const size_t num_inputs,
  const size_t batch_size,
  const size_t num_weight_buffers,
  const size_t num_gpus
...
template <typename T>
void SNIGCapturer<T>::_preprocess(const std::fs::path& input_path) {
...
template <typename T>
void SNIGCapturer<T>::_weight_alloc() {
...
template <typename T>
void SNIGCapturer<T>::_input_alloc() {
...
template <typename T>
void SNIGCapturer<T>::_result_alloc() {
