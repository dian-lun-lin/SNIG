...
#include <SNIG/snig/kernel_wo_resetting.hpp>
...
template <typename T>
class SNIGUpdateWOResetting : public Base<T> {
...
// ----------------------------------------------------------------------------
// Definition of SNIGUpdateWOResetting
// ----------------------------------------------------------------------------

template <typename T>
SNIGUpdateWOResetting<T>::SNIGUpdateWOResetting(
  const dim3& threads,
  const std::fs::path& weight_path,
  const T bias,
  ...
  Base<T>(threads, weight_path, bias, num_neurons_per_layer, num_layers)
{
  ...
  Base<T>::log("Constructing SNIGUpdateWOResetting engine......", "\n");
}
...
template <typename T>
Eigen::Matrix<int, Eigen::Dynamic, 1> SNIGUpdateWOResetting<T>::infer(
  const std::fs::path& input_path,
  const size_t num_inputs,
  const size_t batch_size,
  const size_t num_weight_buffers,
...
template <typename T>
void SNIGUpdateWOResetting<T>::_set_parameters(
  const size_t num_inputs,
  const size_t batch_size,
  const size_t num_weight_buffers,
  const size_t num_gpus
...
template <typename T>
void SNIGUpdateWOResetting<T>::_preprocess(const std::fs::path& input_path) {
      ...
      if(_is_init) {
        ...
        for(size_t cur_layer = 0; cur_layer < Base<T>::_num_layers; cur_layer += _num_weight_buffers) {
          for(size_t k = 0; k < _num_weight_buffers; ++k) {
            ...
            memsets[dev][cur_layer + k] = cfs[dev]->memset(
              _dev_Y[dev][(k + 1) % 2],
              0,
              _batch_ysize
            );
        ...
        for(size_t cur_layer = 0; cur_layer < Base<T>::_num_layers; ++cur_layer) {
          ...
          memsets[dev][cur_layer].precede(infers[dev][cur_layer]);
          ...
          if(cur_layer + 1 < Base<T>::_num_layers) {
            ...
            infers[dev][cur_layer].precede(memsets[dev][cur_layer + 1]);
          }
        }
...
template <typename T>
void SNIGUpdateWOResetting<T>::_weight_alloc() {
...
template <typename T>
void SNIGUpdateWOResetting<T>::_input_alloc() {
...
template <typename T>
void SNIGUpdateWOResetting<T>::_result_alloc() {
