...
class Sequential {
  ...
  private:
    ...
    const T _bias;
	 ...
	 std::vector<Eigen::SparseMatrix<T> > read_weight_and_input (const std::fs::path& input_path) const;
	 Eigen::SparseMatrix<T> read_golden (const std::fs::path& path) const;
	 Eigen::SparseMatrix<T> str_to_matrix(const std::string&s, const int& rows, const int& cols) const;
  ...
  public:
    ...
    ~Sequential();
	 ...
	 std::fs::path weight_path() const { return _weight_path; };
...
template <typename T>
inline 
std::vector<Eigen::SparseMatrix<T> > Sequential<T>::read_weight_and_input (const std::fs::path& input_path) const {
	std::vector<Eigen::SparseMatrix<T> > mats;
	mats.reserve(_num_layers + 1);
	//read input
	std::string input_str = read_file_to_string(input_path);
	mats.push_back(str_to_matrix(input_str, _num_inputs, _num_neurons));
	//read weights
	for(int i = 0; i < _num_layers; ++i){
		...
		mats.push_back(str_to_matrix(data_str, _num_neurons, _num_neurons));
	}
	return mats;
}
template <typename T>
inline
Eigen::SparseMatrix<T> Sequential<T>::str_to_matrix(const std::string& s, const int &rows, const int &cols) const {
...
template <typename T>
inline 
Eigen::SparseMatrix<T> Sequential<T>::read_golden(const std::fs::path& path) const {
	std::string line;
...
void Sequential<T>::infer(
   ...
   std::cout << "Loading the weight and input..............\n";
	...
	auto mats = read_weight_and_input(input);
...
void Sequential<T>::check_passed(const Eigen::SparseMatrix<T>& output_mat, const std::fs::path& golden) const {
	...
	auto golden_mat = read_golden(golden);
